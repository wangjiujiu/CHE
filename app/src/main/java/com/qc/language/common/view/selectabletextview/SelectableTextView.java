...
public class SelectableTextView extends android.support.v7.widget.AppCompatTextView {
    ...
    //是否是变背景色
    private boolean isEnableChangeBackground = false;
    ...
    List<String> clickTextviewList = new ArrayList<>();
    ...
    private ClickableSpan getClickableSpan() {
        return new ClickableSpan() {
            ...
            public void onClick(View widget) {
                ...
                try {
                    word = getText()
                            .subSequence(getSelectionStart(),
                                    getSelectionEnd()).toString();
                    //判断有没有
                    boolean hasChoosed =false;
                    int i =0;
                    int removeposition=0;
                    for(int m=0;m<clickTextviewList.size();m++){
                        hasChoosed =false;
                        i = getSelectionStart();
                        if(Integer.parseInt(clickTextviewList.get(m))==i){
                            //已选中
                            hasChoosed = true;
                            removeposition = m;
                            break;
                    ...
                    if(hasChoosed){
                        ...
                        builder.setSpan(new BackgroundColorSpan(Color.WHITE), getSelectionStart(), getSelectionEnd(), Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
                        setText(builder);
                        clickTextviewList.remove(removeposition);
                    }else{
                        //选中
                        SpannableStringBuilder builder = new SpannableStringBuilder(getText());
                        builder.setSpan(new BackgroundColorSpan(Color.YELLOW), getSelectionStart(), getSelectionEnd(), Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
                        setText(builder);
                        clickTextviewList.add(String.valueOf(i));
                    }
                } catch (Exception e) {
                    e.printStackTrace();
                }
